È un'ottimizzazione che possiamo fare durante l'[analisi del rischio](analisi%20del%20rischio.md) che permette di migliorare la tecnica a cui viene applicata (e.g., [FMECA](FMECA.md#Fuzzy%20if-then)).
...
Una variabile linguistica rappresenta una categoria (e.g., età) e ha associati diversi valori linguistici (e.g., giovane, adulto e anziano; non sono valori numerici come 20, 30).
Ogni valore linguistico ha la sua funzione di appartenenza: dato un input numerico si ottiene un valore che quantitifica quanto esso appartiene a quel valore linguistico; in pratica le funzioni di appartenenza permettono di tradurre un numero di un insieme di probabilità; il numero di queste probabilità è pari al numero di valori linguistici per quella data variabile linguistica.
...
- Operazioni chiuse
	La logica fuzzy è chiusa rispetto alle operazioni di somma e prodotto: se sommiamo/moltiplichiamo due variabili triangolari/trapezoidali otteniamo ancora variabili triangolari/trapezoidali.
- Intersezione
