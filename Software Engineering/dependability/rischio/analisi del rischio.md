L'analisi del rischio è la prima fase della pipeline per il processo di [gestione del rischio](gestione%20del%20rischio.md): studia l'utilizzo del sistema, le performance e si rappresentano i vari rischi.
# Parametri
Il rischio deve essere rappresentato in due dimensioni.
##### Qualitativo
È la dimensione che rappresenta l'effetto del [danno](rischio.md#Danno%20(harm)) rispetto al target, cioè a chi lo subisce: il danno a una persona è molto più rilevante rispetto a quello su un oggetto; il danno all'ambiente è più importante di quello subito da un oggetto e meno di quello su una persona.
##### Quantitativo
Stiamo valutando i livelli di gravità e la loro probabilità secondo la norma ISO 14971.
- Gravità
	In ordine abbiamo: catastrofico, che causa il decesso; critico, che compromette permanentemente; grave, che causa una lesione che richiede un intervento; minore, che causa una lesione che non richiede intervento; trascurabile, che causa disagio.
- Probabilità
	La probabilità di accadimento di un evento non è un singolo valore, ma viene definita pesando due fattori: il primo rappresenta la probabilità che avvenga un guasto e questa si può stimare da valori statistici (i.e., da dati storici); la seconda è la probabilità che tale guasto porti al danno e questa è molto complessa da stimare perché dipende da variabili non controllabili.
	In ordine abbiamo: frequente, $\ge 10^{-3}$; probabile, $10^{-3}\le x\le 10^{-4}$; grave, occasionale, $10^{-4}\le x\le 10^{-5}$; remoto, $10^{-5}\le x\le 10^{-6}$; improbabile, $\le 10^{-6}$. In realtà i valori soglia dipendono anche dal dominio: per sistemi elettronici/meccanici ci si ferma a circa $10^{-6}$ per farmaci circa a $10^{-10}$.
	La probabilità non sempre è facile da capire; si utilizzano dati storici e l'esperienza di chi valuta.
##### Valutazione
Le due dimensioni sopra devono essere incrociate per ottenere una singola immagine del rischio. Ci sono tre tecniche per fare questo:
 - Indici di rischio
	Sono dati medi rappresentati in forma tabellare.
	Sono molto semplici da analizzare ma hanno il limite di non riuscire a rappresentare bene il rischio in un sistema complesso.
 - Rischio puntuale
	Correlano il rischio a un preciso punto geografico.
	Si può parlare di: rischio puntuale, dove chi subisce il danno non ha via di fuga e non può muoversi; rischio individuale, dove chi subisce il danno può muoversi e usare protezioni.
 - Rischio diffuso
	L'idea è la stessa del rischio puntale ma tiene in considerazione anche la tipologia del target (i.e., chi subisce il danno) e la presenza di target particolarmente vulnerabili.
# Tecniche
Le tecniche per eseguire l'analisi del rischio sono molte e dipendono dal contesto e dai dati inizialmente a disposizione.
Ci sono due classi:
- Induttivo
	Si parte dalle cause e si determino le conseguenze.
- Deduttivo
	Si parte dalle conseguenze e si arriva alle cause che le hanno generate.
##### PHA (Preliminary Hazard Analysis)
È una tecnica qualitativa e induttiva, usata soprattutto nelle prime fasi di progettazione dove ancora non abbiamo dati numerici a disposizione.
Si parte dal top event, cioè la causa sotto esame; si stimano le sue conseguenze e a ognuna di questa gli viene assegnato un livello descrittivo (i.e., trascurabile, marginale, critico e catastrofico); i risultati vengono riassunti in una tabella di [questo](PHA%20tabel.png) tipo, molto descrittiva, qualitativa e che rimane molto generica e basata sull'esperienza di chi svolge l'analisi.
- È una tecnica semplice che si svolge in pochi giorni anche per sistemi complessi.
- Permette risolvere le cause del rischio in fase di progettazione con un costo basso.
##### HAZOP (Hazard and Operability Studies)
È definita dalla norma IEC 61882, qualitativa e induttiva usata soprattutto in sistemi complessi.
Il vantaggio è che si ottiene un'analisi completa; lo svantaggio è che rispetto a PHA è molto complessa e richiede tempo e molto personale.
- Prima di analizzare i guasti si deve mappare il sistema e suddividerlo in nodi: i nodi interni sono componenti del sistema che svolgono il loro compito all'interno di un perimetro ben definito (e.g., serbatoio o un modulo software isolato); i nodi di frontiera rappresentano componenti che interagiscono con l'esterno (e.g., valvola o API software).
- Per ogni nodo si deve definire cosa dovrebbe fare usando dei parametri, i quali dipendono da applicazione in applicazione (e.g., per un software latenza, throughtput, input): i parametri sono in pratica le funzioni che il nodo svolge.
- Si associa una keyword a ogni parametro per ottenere delle deviazioni, cioè un comportamento indesiderato di un parametro. Le keyword sono definite dallo standard: NO, PIÙ, DIVERSO, PRIMA, DOPO. 
- Per ogni deviazione si deve compilare una tabella che descrive la catena deviazione-causa-conseguenza-rimedio: la causa della deviazione, cioè perché è successo; la consegue che traspare al sistema; il rimedio che abbiamo già implementato o che dobbiamo fare.
##### ETA (Event Tree Analysis)
È una tecnica induttiva e quantitativa definita dalla norma IEC 62502.
Si basa su un albero binario: a partire da un evento iniziale si cerca di mettere delle barriere (i.e., delle guardie per arginare il problema); il ramo destro rappresenta il successo della barriera e gli è assegnata una probabilità $P(A)$, il ramo sinistro il suo fallimento con probabilità $1-P(A)$; percorrendo l'albero e attraversando le possibili barriere si arriva alle foglie dove sono rappresentate le possibili conseguenze. Possiamo in questo modo calcolare la probabilità di ogni singola conseguenza usando la probabilità congiunta, cioè moltiplicando le probabilità del percorso.
Il vantaggio è che possiamo vedere quale barriera ha salvato il sistema, capire quanto è il rapporta beneficio/costo di aggiungere una barriera ed è semplice calcolare le probabilità di finire in una conseguenza. Lo svantaggio è che per ogni evento iniziale si deve costruire un albero a parte.
Nella sua versione [fuzzy](logica%20fuzzy.md) le probabilità di accadimento sono espresse dalle funzioni di appartenenza e la probabilità della conseguenza finale (i.e., del nodo foglia) è rappresentata dall'output della defatizzazione.
##### [FMEA](FMEA.md)
##### [FMECA](FMECA.md)
##### [FTA](FTA.md)