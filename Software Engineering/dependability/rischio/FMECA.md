La Failure Modes and Effects and Criticality Analysis (FMECA) è una tecnica per l'[analisi del rischio](analisi%20del%20rischio.md) semi-quantitativa, che rispetto alla [FMEA](FMEA.md) aggiunge un'analisi delle criticità (dopo l'[analisi degli effetti](FMEA.md#Fasi)).
...
La tecnica assegna un peso peso ai modi di guasto e ai loro effetti.
...
- Occurrence (O)
	È la probabilità che si verifichi il guasto durante la vita del sistema. È strettamente legato al tasso di guasto.
...
##### Occurrence
Il modo più semplice di limitare il $RPN$ è lavorare sull'occurrence. Ci sono quattro modi per farlo:
- Manutenzione preventiva.
- Usare componenti di qualità migliore. In questo modo si abbassa il tasso di guasto.
- Utilizzare derating per ridurre il tasso di guasto.
- Ridondanza.
##### Detection
Possiamo ridurre il $RPN$ agendo monitorando di più il sistema e quindi migliorando la diagnostica.
Il problema è che diminuire il valore della detection probabilmente aumentare i costi e il tasso di guasto (perché si aggiungono componenti) e quindi si peggiora l'occurrence; dobbiamo trovare un equilibrio.
# Conseguenze
Vediamo vantaggi e svantaggi della FMECA.
##### Vantaggi
- Migliora la [dependability](dependability.md) del sistema.
- Determina la necessità di ridondanze, il livello di utilizzo del derating o semplificazione del progetto.
- Documenta il rischio del sistema e dei singoli componenti.
- Si stabiliscono le attività di manutenzione.
##### Svantaggi
- Il valore $RPN$ va da 1 a 1000, ma in realtà ha valori molto sparsi: ad esempio da 900 ($10^2\times{9}$) a 1000 ($10^3$) non ha valori; in pratica non abbiamo 1000 valori ma solo 120. Inoltre si ottengono gli stessi valori di $RPN$ con combinazioni diverse. Per risolvere questi problemi ci sono delle soluzioni (quasi mai migliori in ambito industriale):
	...
	- $URPN$: utilizza l'esponenziale nella severity, dandole molta più importanza rispetto alle altre.
...
##### Step
La pipeline per ottenere il RPN fuzzy è la sequente. Gli input a questo processo sono le tre variabili occurance, severity e detection.
...
# Fuzzy aritmico
La parte più complessa è quella di definire tutte le regole $if-then$, il cui numero esplode al crecere dei valori linguistici delle variabili.
Per ovviare a questo problema possiamo usare direttamente le operazioni matematiche sul valore delle variabili linguistiche (ognuna con i suoi valori linguistici) ottenuto dopo la fuzzizazione.
##### Pipeline
- Si definiscono dei pesi $(w_O​,w_S​,w_D​)$ per i valori degli input, i.e. per i valori che poi useremo di occurance, severity e detection.
- Ogni valore trovato per $O$, $S$ e $D$ viene fuzzificato e trasformato in una variabile linguistica.
- I valori in $[0,1]$ delle variabili linguistiche vengono pesati in modo esponenziale per i loro pesi (e.g. si fa $O^{w_{O}}$).
