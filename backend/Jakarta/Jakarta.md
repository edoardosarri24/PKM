La Java Enterprise Edition JEE, ora nota come Jakarta EE (prima JEE era in mano alla Oracle e parzialmente open source, ora è della Eclipse Foundation ed è open source), è una piattaforma di stardard e API open source per lo sviluppo di applicazioni distribuite, scalabili e sicure in contesti aziendali in Java.
- La JSE è la version base di Java, che fornisce le librerie essenziali per lo sviluppo di applicazioni stand alone che vengono eseguite in una JVM in locale e che non hanno idea di scalare per numero di utenti gestiti.
- La JEE estende la JSE con API e specifiche. Il suo backend viene eseguito su un qualche application server (e.g., WildFly, GlassFish o Tomcat).
##### Flusso nell'architettura
Vediamo il flusso tipi con cui funziona Jakarta.
- L'utente richiede un'operazione tramite il presentation layer.
- La richiesta viene passata alla business logic o service layer (a seconda del tipo di applicazione) che la valida e richiede la creazione delle istanze degli oggetti al DAO giusto.
- Il DAO usa l'entity manager per recuperare lo stato delle istanze, che poi crea iniettando tale stato.
- Gli oggetti vengono utilizzati per performare l'operazione. Alla fine si memorizzano eventuali cambiamenti di stato nel database.
# Presentation layer
...
Quello che facciamo è quindi scambiare oggetti leggeri rappresentati in formato json; tali oggetti sono istanze di classi semplificate rispetto a quelle del domain. Gli oggeti DTO sono restituiti dai metodi del domain model.
...
##### Tecnologie
Le tecnologie (framework e API) utilizzate sono:
- JSF (Java Server Faces)
	Framework MVC (Model View Controller) per creare interfacce web strutturate con componenti riutilizzabili. Si usa nell'architettura stateful.
- Angular2
	Framework per creare frontend complessi nell'architettura restful.
...
##### Tecnologie
- Jakarta EJB (Enterprise JavaBeans)
	Permette di definire componenti transazionali e scalabili.
- [CDI](Context%20and%20Depencency%20Injection%20(CDI).md) (Contexts and Dependency Injection)
# Persistence layer
