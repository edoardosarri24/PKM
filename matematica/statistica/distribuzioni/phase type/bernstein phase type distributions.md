I Bernstein phase type (BPH) distributions sono [phase type distribution](phase%20type%20distribution.md) che utilizzano i polinomi di berstein usando la [base esponenziale](polinomi%20di%20Bernstein.md#Esponenziale).
# Vantaggi
Oltre ai [vantaggi dei polinomi](polinomi%20di%20Bernstein.md#Vantaggi) abbiamo:
...
##### Intevallo $[a,b]$
L'approssimazione è data dalla valutazione del polinomio in $\tfrac{x-a}{b-a}$, cioè usando la base [lineare](polinomi%20di%20Bernstein.md#Lineare) e allora abbiamo  $B_{n}(F,\tfrac{x-a}{b-a})=\displaystyle \sum_{i=0}^n F(a+\tfrac{i}{n}(b-a))\binom{n}{i}\tfrac{(x-a)^n(b-x)^{n-i}}{(b-a)^n}$, con $x\in[a,b]$.
##### Intervallo $[0,+\infty]$
...
##### Intervallo $[0,1]$
L'approssimazione è data dalla derivata $B_{n}'(F,x)=\displaystyle \sum_{i=1}^n\left( F(\tfrac{i}{n})-F(\tfrac{i-1}{n})\right)n\binom{n}{i}x^{i-1}(1-x)^{n-i}$, con $x\in[0,1]$.
##### Intervallo $[a,b]$
